...
export default function NotebookPage() {
  const { diaries, user, saveExpression, reviewExpression } = useAppStore()
  ...
  return (
    <div className="page-stack">
      ...
      <section className="panel wide">
        ...
        <div className="expression-bank">{(user.savedExpressions || []).map(expression => <span key={expression}>{expression}<button onClick={() => reviewExpression(expression)}>{user.reviewedExpressions?.includes(expression) ? 'Reviewed' : '+5 Review'}</button></span>)}</div>
      </section>
